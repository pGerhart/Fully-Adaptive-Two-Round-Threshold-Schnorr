...
impl Polynomial {
    ...
    pub fn commit(&self, pp: &Params, rho_f: Scalar) -> RistrettoPoint {
        ...
        msm(&pp.g, &coeffs) + pp.g_rho * rho_f
    }

    /// Produce (y, proof, public inputs) for f(z).
    pub fn prove_eval(&self, z: Scalar, domain: &str) -> (Scalar, LinearProof, PublicEval) {
        let n = self.coeffs.len();
        let m = next_pow2(n);
        let pp = Params::setup(m, domain);
        ...
        // pad coeffs to m
        let mut x = self.coeffs.clone();
        x.resize(m, Scalar::ZERO);

        let Cf = msm(&pp.g, &x) + pp.g_rho * rho_f;
...
pub fn verify_eval(public: &PublicEval, y: Scalar, proof: &LinearProof) -> bool {
